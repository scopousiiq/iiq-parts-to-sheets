...
### Changed
- **Group 3 fast-forward + throttle fix** (`scripts/InventoryActions.gs`): on a fresh run the loader binary-searches (~8 probe calls) for the first page reaching the earliest Group 2 ticket date and skips the older pages entirely — safe because actions sort `CreatedDate Ascending` and cannot predate their ticket. On the kcs tenant this cuts a full Group 3 pass (and every nightly refresh) from ~142 calls to ~30. Also removed a per-page `Utilities.sleep` that double-throttled on top of `apiRequest()`'s built-in `THROTTLE_MS` delay.
...
### Added
- `.clasp.test.json` — second clasp target for the temporary iteration test sheet (push with `clasp -P .clasp.test.json push -f`), matching the sibling repos' convention.
